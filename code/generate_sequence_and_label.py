...
def main():
    ...
    # Output numpy file
    output_sequence_numpy = "data/sequence.npz"
    output_label_numpy = "data/label.npz"
    ...
    if all_sequences_list and all_labels_list:
        ...
        np.savez_compressed(output_sequence_numpy, sequences=final_sequences)
        np.savez_compressed(output_label_numpy, labels=final_labels)
...
def create_sequences(tracking_data, annotation_data, sequence_length=20, fps=5):
    frame_step = 25 // fps
    num_frames = sequence_length * fps
    sequence_data = []
    label_data = []

    for idx in range(25 * 10, len(tracking_data) - 25, frame_step):
        current_time = tracking_data.iloc[idx]['match_time']
        start_time = current_time - sequence_length * 1000

        # Extract past 20 seconds of data (or pad with zeros)
        past_data = tracking_data[(tracking_data['match_time'] >= start_time) & (tracking_data['match_time'] <= current_time)]
        if len(past_data) < num_frames:
            padding = pd.DataFrame(0, index=np.arange(num_frames - len(past_data)), columns=tracking_data.columns)
            past_data = pd.concat([padding, past_data])

        # Resample to 5 fps
        sampled_data = past_data.iloc[-num_frames::frame_step, 1:].values  # Exclude match_time
        sequence_data.append(sampled_data)

        # Find corresponding annotation data
        label = annotation_data[annotation_data['match_time'] == current_time].iloc[:, 1:].values  # Exclude match_time
        if len(label) > 0:
