...
def make_resulting_video(input_video_path, output_video_path, resulting_df):
    ...
    columns_labels = [col for col in resulting_df.columns if not col.startswith("output_")]
    ...
    labels_1 = resulting_df[[col for col in columns_labels if col.endswith("1")]].copy()
    labels_2 = resulting_df[[col for col in columns_labels if col.endswith("2")]].copy()
    ...
    while True:
        ...
        if frame_count >= frame_rate * 9:
            index = (frame_count - int(frame_rate * 9)) // 5
            index = min(max(0, index), len(outputs_1) - 1)
            
            output_row_1, output_row_2 = outputs_1.iloc[index], outputs_2.iloc[index]
            ...
            for i, tactic in enumerate(tactics_list):
                ...
                for x_position, output_row, label_row in zip([x_1_position, x_2_position], [output_row_1, output_row_2], [label_row_1, label_row_2]):
                    ...
                    if output < 0.5 and label >= 0.5:
                        ...
                        text_color = (255, 0, 255)  # ピンク色
                    else:
                        text_color = (255, 255, 255)  # 白色（デフォルト）
